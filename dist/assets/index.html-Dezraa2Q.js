...
`,ze=class extends q{static styles=[$,Re];static properties={tool:{attribute:!1},pageContext:{attribute:!1},exportInProgress:{type:Boolean},pendingToolId:{attribute:!1}};constructor(){super(),this.tool={},this.pageContext={type:`loading`},this.exportInProgress=!1,this.pendingToolId=null}get toolViewModel(){return Ie(this.tool,{pageContext:this.pageContext,exportInProgress:this.exportInProgress,pendingToolId:this.pendingToolId})}activate(){let{id:e,disabled:t}=this.toolViewModel;t||!e||this.dispatchEvent(new CustomEvent(`popup-tool-activate`,{detail:{toolId:e},bubbles:!0,composed:!0}))}render(){let{id:e,title:t,description:n,appearance:r,busyLabel:i,disabled:a,reason:o,busy:s}=this.toolViewModel;return I`
            <button
                type="button"
                data-tool-id=${e}
        ...
        `}};customElements.define(`popup-tool-card`,ze);var Be=class extends q{static styles=[$,Le];static properties={title:{type:String},tools:{attribute:!1},pageContext:{attribute:!1},exportInProgress:{type:Boolean},pendingToolId:{attribute:!1}};constructor(){super(),this.title=``,this.tools=[],this.pageContext={type:`loading`},this.exportInProgress=!1,this.pendingToolId=null}render(){return I`
        ...
        `}};customElements.define(`popup-tool-group`,Be);async function Ve(){let[e]=await chrome.tabs.query({active:!0,currentWindow:!0});return Me(e)}async function He(){return!!(await chrome.storage.local.get(`exportInProgress`)).exportInProgress}function Ue(e,t){let n={action:t};return De(n)?new Promise((t,r)=>{chrome.tabs.sendMessage(e,n,e=>{chrome.runtime.lastError?r(Error(chrome.runtime.lastError.message)):t(e)})}):Promise.reject(Error(`Unsupported Toolbox command.`))}function We(e){let t=t=>{Oe(t)&&e(t)};return chrome.runtime.onMessage.addListener(t),()=>chrome.runtime.onMessage.removeListener(t)}var Ge=class extends q{static styles=[$,be];static properties={pageContext:{state:!0},initialized:{state:!0},exportInProgress:{state:!0},pendingToolId:{state:!0},status:{state:!0}};constructor(){super(),this.pageContext={type:`loading`},this.initialized=!1,this.exportInProgress=!1,this.pendingToolId=null,this.status=null,this.connectionVersion=0,this.exportStatusObserved=!1,this.unsubscribeFromExportStatus=null}connectedCallback(){super.connectedCallback();let e=++this.connectionVersion;this.exportStatusObserved=!1,this.unsubscribeFromExportStatus=We(e=>{this.handleExportStatus(e)}),this.initialize(e)}disconnectedCallback(){this.connectionVersion+=1,this.unsubscribeFromExportStatus?.(),this.unsubscribeFromExportStatus=null,super.disconnectedCallback()}async initialize(e){let[t,n]=await Promise.allSettled([Ve(),He()]);e!==this.connectionVersion||!this.isConnected||(this.pageContext=t.status===`fulfilled`?t.value:{type:`unavailable`},n.status===`fulfilled`&&!this.exportStatusObserved&&(this.exportInProgress=n.value),this.initialized=!0)}handleExportStatus(e){this.exportStatusObserved=!0,this.exportInProgress=e.state===X.STARTED,e.state===X.COMPLETE?this.status={message:`Экспорт завершён.`,isError:!1}:e.state===X.ERROR&&(this.status={message:e.message||`Не удалось экспортировать марафон.`,isError:!0})}async executeTool(e){let t=Fe(e);if(!t||Pe(t,this.pageContext)||!this.pageContext.tabId||this.exportInProgress||this.pendingToolId!==null)return;this.status=null,this.pendingToolId=t.id,t.id===`marathon-export`&&(this.exportInProgress=!0);let n=this.connectionVersion;try{if(await Ue(this.pageContext.tabId,t.command),n!==this.connectionVersion||!this.isConnected)return;this.pendingToolId=null,t.closeOnSuccess&&window.close()}catch(e){if(n!==this.connectionVersion||!this.isConnected)return;t.id===`marathon-export`&&(this.exportInProgress=!1),this.pendingToolId=null,this.status={message:e.message||`Не удалось запустить инструмент.`,isError:!0}}}handleToolActivate(e){this.executeTool(e.detail.toolId)}render(){let e=Ne(this.pageContext);return I`
        ...
        `}};customElements.define(`popup-app`,Ge),t(document.documentElement);