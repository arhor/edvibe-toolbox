...
`;var Re=`toolfox-telegram-owned-groups-dialog`;function ze(e){if(!e)return`Нет данных об активности`;let t=new Date(e);return Number.isNaN(t.getTime())?`Нет данных об активности`:new Intl.DateTimeFormat(void 0,{dateStyle:`medium`,timeStyle:`short`}).format(t)}function Be(e){return e===`supergroup`?`Супергруппа`:`Группа`}function Ve(e){return e===!0?`Можно отправлять сообщения`:e===!1?`Отправка сообщений недоступна`:`Возможность отправки неизвестна`}var He=class extends K{static styles=[Pe,Fe,Ie,Le,je];static properties={options:{state:!0},groups:{state:!0},viewState:{state:!0},message:{state:!0}};constructor(){super(),this.options=null,this.groups=[],this.viewState=`loading`,this.message=`Загружаем группы текущего аккаунта…`,this.loadPromise=null,this.handleKeydownBound=e=>{e.key===`Escape`&&this.close()}}configure(e={}){return this.options=e&&typeof e==`object`?e:{},this.isConnected&&this.load(),this}connectedCallback(){super.connectedCallback(),this.ownerDocument?.addEventListener(`keydown`,this.handleKeydownBound),this.load()}disconnectedCallback(){this.ownerDocument?.removeEventListener(`keydown`,this.handleKeydownBound),super.disconnectedCallback()}async load(){if(!this.options?.onLoad||this.loadPromise)return this.loadPromise;this.viewState=`loading`,this.groups=[],this.message=`Загружаем группы текущего аккаунта…`,this.loadPromise=(async()=>{try{let e=await this.options.onLoad();this.groups=[...e?.groups||[]],this.viewState=e?.state||(this.groups.length>0?`ready`:`empty`),this.message=this.viewState===`unsupported`?`Текущая версия Telegram Web K не предоставляет ожидаемый runtime-интерфейс.`:this.viewState===`empty`?`У текущего аккаунта не найдено созданных им активных групп.`:``}catch(e){this.groups=[],this.viewState=`error`,this.message=e instanceof Error?e.message:`Не удалось загрузить группы Telegram.`}finally{this.loadPromise=null}})(),await this.loadPromise,await this.updateComplete,this.shadowRoot?.querySelector(`[data-action="close"]`)?.focus()}retry(){this.load()}close(){this.options?.onClose?.()}renderState(){let e=this.viewState===`error`,t=e||this.viewState===`unsupported`,n=this.viewState===`loading`?`Загрузка…`:this.viewState===`empty`?`Групп пока нет`:e?`Не удалось загрузить группы`:`Telegram Web K несовместим`;return F`
        ...
        `}renderGroup(e){return F`
            <li class="group-card" data-peer-id=${String(e.peerId)}>
                <div class="group-heading">
                    ...
                    <span class="kind">${Be(e.kind)}</span>
                </div>
                <div class="metadata">
                    <span>${ze(e.lastActivityAt)}</span>
                    <span>${Ve(e.canSendText)}</span>
                </div>
            </li>
        ...
        `}};customElements.define(Re,He);var Ue=`supported`,We=new Set([`group`,`supergroup`]);function Ge(e){return!e||!e.isActive||!e.isCreator||e.canSendText===!1||!We.has(e.groupType)||!Number.isFinite(Number(e.peerId))?null:Object.freeze({canSendText:typeof e.canSendText==`boolean`?e.canSendText:null,isCreator:!0,kind:e.groupType,lastActivityAt:e.lastActivityAt||null,peerId:Number(e.peerId),title:String(e.title||``).trim()||`Без названия`})}function Ke(e){return Object.freeze({groups:Object.freeze([]),reason:e?.reason||`unsupported-runtime`,state:`unsupported`})}async function qe(e,{pageSize:t=100}={}){if(!e||typeof e.getCompatibility!=`function`)throw TypeError(`Telegram adapter is required.`);let n=e.getCompatibility();if(n?.state!==Ue)return Ke(n);let r=new Map,i=new Set,a=0;for(;a!==null;){if(i.has(a))throw Error(`Telegram dialog pagination repeated an offset.`);i.add(a);let n=await e.listDialogs({limit:t,offset:a});for(let t of n.items||[]){if(!Number.isFinite(Number(t.peerId))||Number(t.peerId)>=0)continue;let n=Ge(await e.resolveGroupCandidate(t));n&&r.set(n.peerId,n)}a=n.nextOffset}let o=Object.freeze([...r.values()]);return Object.freeze({groups:o,reason:null,state:o.length===0?`empty`:`ready`})}function Je({adapter:e,documentApi:t=globalThis.document,logger:n={log(){}}}){if(!e)throw TypeError(`Telegram adapter is required.`);if(!t?.createElement)throw TypeError(`Document API is required.`);let r=null;function i(){r?.remove(),r=null}function a(){return r?.isConnected?(r.load?.(),r):(r=t.createElement(Re),r.configure({onClose:i,onLoad:()=>qe(e)}),(t.body||t.documentElement).append(r),n.log(`Telegram owned-group browser opened.`),r)}return Object.freeze({close:i,open:a})}var Ye=Object.freeze({type:t.OPEN_TELEGRAM_GROUP_BROWSER,create(e){let t=Je({adapter:e.telegramWeb,logger:e.logger.createChildLogger(`TelegramOwnedGroups`)});return()=>t.open()}}),q=Object.freeze({EDVIBE:`edvibe`,TELEGRAM_WEB_K:`telegram-web-k`,UNSUPPORTED:`unsupported`});function Xe(e){return String(e||``).toLowerCase().replace(/\.$/,``)}function Ze(e=globalThis.location){let t=Xe(e?.hostname),n=String(e?.pathname||`/`);return t===`edvibe.com`||t.endsWith(`.edvibe.com`)?q.EDVIBE:t===`web.telegram.org`&&(n===`/k`||n.startsWith(`/k/`))?q.TELEGRAM_WEB_K:q.UNSUPPORTED}var J=Object.freeze({SUPPORTED:`supported`,UNSUPPORTED:`unsupported`}),Qe=Object.freeze({MANAGER_FACTORY_UNAVAILABLE:`manager-factory-unavailable`,MANAGER_FACADE_UNAVAILABLE:`manager-facade-unavailable`}),Y=class extends Error{constructor(e,{code:t,cause:n,compatibility:r,operation:i}={}){super(e,{cause:n}),this.name=`TelegramWebKRuntimeError`,this.code=t,this.compatibility=r,this.operation=i}};function X(e){return typeof e==`object`&&!!e&&!Array.isArray(e)}function Z(e){if(e==null||e===``)return null;let t=Number(e);return Number.isFinite(t)?t:null}function $e(e){let t=Number.parseInt(String(e||`1`),10);return t>=1&&t<=4?t:1}function et(e=globalThis.location){try{return $e(new URL(e?.href||`https://web.telegram.org/k/`).searchParams.get(`account`))}catch{return 1}}function tt(e,t){return Object.freeze({accountNumber:t,reason:e,state:J.UNSUPPORTED})}function nt(e){return Object.freeze({accountNumber:e,reason:null,state:J.SUPPORTED})}function rt({globalObject:e=globalThis,location:t=globalThis.location}={}){let n=et(t);return typeof e?.createProxiedManagersForAccount==`function`?nt(n):tt(Qe.MANAGER_FACTORY_UNAVAILABLE,n)}function it(e){if(!X(e))return null;let t=Z(e.peerId);if(t===null)return null;let n=Z(e.top_message);return Object.freeze({peerId:t,topMessageId:n})}function at(e){return e?._===`chat`?`group`:e?._===`channel`&&e.pFlags?.megagroup?`supergroup`:e?._===`channel`?`channel`:e?._===`user`?`user`:`unknown`}function ot(e,t){if(t!==`group`&&t!==`supergroup`)return!1;let n=e?.pFlags||{};return n.left||n.deactivated?!1:!(t===`group`&&e?.migrated_to)}function st(e,t){if(!X(t))return Object.freeze({isActive:!1,isBroadcast:!1,isCreator:!1,isGroup:!1,peerId:e,title:``,type:`unknown`});let n=at(t);return Object.freeze({isActive:ot(t,n),isBroadcast:n===`channel`&&!!t.pFlags?.broadcast,isCreator:!!t.pFlags?.creator,isGroup:n===`group`||n===`supergroup`,peerId:e,title:typeof t.title==`string`?t.title:``,type:n})}function ct({canSendText:e=null,dialog:t,lastActivityAt:n=null,peer:r}){return!r?.isGroup||!t?null:Object.freeze({canSendText:typeof e==`boolean`?e:null,groupType:r.type,isActive:!!r.isActive,isCreator:r.isCreator,lastActivityAt:n,peerId:r.peerId,title:r.title})}function lt(e){let t=Z(e?.date);return t===null||t<0?null:new Date(t*1e3).toISOString()}function Q(e,t){return new Y(`Telegram Web K operation "${e}" failed.`,{cause:t,code:`TELEGRAM_WEB_K_RUNTIME_CALL_FAILED`,operation:e})}var ut=class{constructor({globalObject:e=globalThis,location:t=globalThis.location}={}){this.globalObject=e,this.location=t,this.cachedManagers=null,this.cachedAccountNumber=null,this.cachedManagerFactory=null}getCompatibility(){return rt({globalObject:this.globalObject,location:this.location})}getManagers(){let e=this.getCompatibility();if(e.state!==J.SUPPORTED)throw new Y(`Telegram Web K runtime adapter is unavailable.`,{code:`TELEGRAM_WEB_K_UNSUPPORTED_RUNTIME`,compatibility:e,operation:`resolve-runtime`});let t=this.globalObject.createProxiedManagersForAccount;if(this.cachedManagers&&this.cachedAccountNumber===e.accountNumber&&this.cachedManagerFactory===t)return this.cachedManagers;let n;try{n=t(e.accountNumber)}catch(e){throw Q(`resolve-runtime`,e)}if(!n||typeof n!=`object`&&typeof n!=`function`)throw new Y(`Telegram Web K manager facade is unavailable.`,{code:`TELEGRAM_WEB_K_UNSUPPORTED_RUNTIME`,compatibility:tt(Qe.MANAGER_FACADE_UNAVAILABLE,e.accountNumber),operation:`resolve-runtime`});return this.cachedManagers=n,this.cachedAccountNumber=e.accountNumber,this.cachedManagerFactory=t,n}async listDialogs({limit:e=50,offset:t=0}={}){let n=Math.max(1,Math.trunc(e)),r=Math.max(0,Math.trunc(t));try{let e=await this.getManagers().dialogsStorage.getDialogs({limit:n,offsetIndex:r});if(!X(e)||!Array.isArray(e.dialogs))throw TypeError(`Unexpected Telegram dialog page shape.`);let t=e.dialogs.length,i=e.dialogs.map(it).filter(Boolean),a=Z(e.count),o=t===0||a!==null&&r+t>=a?null:r+t;return Object.freeze({count:a,items:Object.freeze(i),nextOffset:o})}catch(e){throw e instanceof Y?e:Q(`list-dialogs`,e)}}async resolvePeer(e){let t=Z(e);if(t===null)throw TypeError(`peerId must be a finite number`);try{return st(t,await this.getManagers().appPeersManager.getPeer(t))}catch(e){throw e instanceof Y?e:Q(`resolve-peer`,e)}}async getLastActivity({peerId:e,topMessageId:t}){if(t==null)return null;try{return lt(await this.getManagers().appMessagesManager.getMessageByPeer(e,t))}catch(e){throw e instanceof Y?e:Q(`get-last-activity`,e)}}async canSendText(e){let t=Z(e);if(t===null||t>=0)return!1;try{return!!await this.getManagers().appChatsManager.hasRights(Math.abs(t),`send_plain`)}catch(e){throw e instanceof Y?e:Q(`check-send-text`,e)}}async resolveGroupCandidate(e){let t=await this.resolvePeer(e.peerId);if(!t.isGroup)return null;if(!t.isActive||!t.isCreator)return ct({dialog:e,peer:t});let[n,r]=await Promise.all([this.getLastActivity(e),this.canSendText(e.peerId)]);return ct({canSendText:r,dialog:e,lastActivityAt:n,peer:t})}async sendText(e,t){let n=Z(e);if(n===null)throw TypeError(`peerId must be a finite number`);if(typeof t!=`string`||t.trim()===``)throw TypeError(`text must be a non-empty string`);try{return await this.getManagers().appMessagesManager.sendText({peerId:n,text:t}),Object.freeze({ok:!0})}catch(e){throw e instanceof Y?e:Q(`send-text`,e)}}async probe(){let e=this.getCompatibility();if(e.state!==J.SUPPORTED)return e;try{let t=await this.listDialogs({limit:1,offset:0});if(t.items.length>0){let e=t.items[0];await this.resolvePeer(e.peerId),await this.getLastActivity(e)}return Object.freeze({...e,capabilities:Object.freeze({dialogDiscovery:!0,lastActivity:!0,peerResolution:!0}),requiresLiveValidation:Object.freeze([`sendText`])})}catch(t){return Object.freeze({accountNumber:e.accountNumber,reason:`runtime-contract-rejected`,state:J.UNSUPPORTED,errorCode:t?.code||`TELEGRAM_WEB_K_RUNTIME_CALL_FAILED`})}}};function dt(e){return new ut(e)}var ft=class{constructor({globalObject:e=globalThis,location:t=globalThis.location,logger:n}){this.logger=n,this.telegramWeb=dt({globalObject:e,location:t}),this.dispatch=null,this.registerDispatch=this.registerDispatch.bind(this)}registerDispatch(e){if(typeof e!=`function`)throw TypeError(`dispatch must be a function`);if(this.dispatch!==null)throw Error(`dispatch is already registered`);this.dispatch=e}},pt=class e{constructor({namespace:e,namespaces:t=null}){this.namespaces=t??[mt(e)],this.log=this.log.bind(this),this.createChildLogger=this.createChildLogger.bind(this)}log(...e){console.log(this.namespaces.map(e=>`[${e}]`).join(``),...e)}createChildLogger(t){return new e({namespaces:[...this.namespaces,mt(t)]})}};function mt(e){if(typeof e!=`string`||!e.trim())throw Error(`Namespace must be a non-empty string.`);return e}var $=new pt({namespace:`MAIN:TELEGRAM_WEB_K`});if(Ze(window.location)===q.TELEGRAM_WEB_K){let e=new d({context:new ft({globalObject:window,location:window.location,logger:$}),features:[Ye]});window.addEventListener(`message`,({source:t,data:n})=>{t===window&&e.dispatch(n)}),$.log(`Toolfox Telegram Web K runtime ready.`)}else $.log(`Telegram Web K MAIN entry loaded on an unsupported page.`)})();