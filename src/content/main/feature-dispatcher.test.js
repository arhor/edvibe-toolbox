...
import { withMainBrowserEnvironment } from '#src/content/main/main-context-test-fixtures.js';
...
function findChildLoggers(logger, namespace) {
    return logger.childLoggers.filter((childLogger) => childLogger.namespace === namespace);
}

function createDispatcherHarness(features, logger = createLogger()) {
    const { context, dispatcher } = withMainBrowserEnvironment(() => {
        const mainContext = new MainContext({ logger });
        return {
            context: mainContext,
            dispatcher: new FeatureDispatcher({ context: mainContext, features })
        };
    });
    return {
        context,
        dispatcher,
        dispatcherLogger: findChildLoggers(logger, 'FeatureDispatcher')[0],
        logger
    };
}

describe('FeatureDispatcher', () => {
    test('should require a context with logger and dispatch registration', () => {
        // Given
    ...
    test('should create its own child logger', () => {
        // Given
        const harness = createDispatcherHarness([]);

        // When
        const dispatcherLoggers = findChildLoggers(harness.logger, 'FeatureDispatcher');

        // Then
        assert.equal(dispatcherLoggers.length, 1);
        assert.equal(harness.dispatcherLogger, dispatcherLoggers[0]);
    });

    test('should reject feature definitions when type or create is invalid', () => {
        // Given
        const harness = createDispatcherHarness([]);
    ...
    test('should reject registration when feature type is already registered', () => {
        // Given
        const definition = {
    ...
    test('should reject registration when feature factory does not create a handler', () => {
        // Given
        const harness = createDispatcherHarness([]);
        const register = () => harness.dispatcher.register({
            type: WINDOW_MESSAGE_TYPES.OPEN_ACTION_RECORDER,
    ...
    test('should register dispatch before creating features with the same context', () => {
        // Given
        ...
        assert.equal(contexts[0].transport, harness.context.transport);
        assert.equal(contexts[0].operationGuard, harness.context.operationGuard);
    ...
    test('should ignore messages when message contract is invalid', () => {
        // Given
        let calls = 0;
        const harness = createDispatcherHarness([{
            type: WINDOW_MESSAGE_TYPES.OPEN_ACTION_RECORDER,
    ...
    test('should return false when valid command has no registered handler', () => {
        // Given
        const harness = createDispatcherHarness([{
            type: WINDOW_MESSAGE_TYPES.OPEN_ACTION_RECORDER,
            create: () => () => { }
    ...
    test('should dispatch message when command type is registered', () => {
        // Given
    ...
    test('should isolate synchronous failure when later command is dispatched', () => {
        // Given
    ...
    test('should isolate asynchronous failure when handler promise rejects', async () => {
        // Given
