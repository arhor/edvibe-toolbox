...
} from '#src/content/main/features/batch-user-onboarding/batch-user-onboarding-history.js';
...
test('buildExecutionHistoryInput should record one successful function call when execution returns', () => {
    // Given
    const plan = { rows: [{ email: 'user@example.test' }], targetModerator: null };
    const parameters = {
        plan,
        marathonId: 42,
        requestDelayMs: 100,
    ...
    const result = { plan, rows: [], fatalError: { code: 'STATE_CHANGED' } };
    ...
    assert.deepEqual(record.results[0].data, {
        parameters: { plan, marathonId: 42, requestDelayMs: 100 },
        result
    });
});
...
test('recorded execution should persist only after executePlan returns', async () => {
    // Given
    const events = [];
    const parameters = { plan: {}, marathonId: 42 };
    const result = { rows: [], fatalError: null };
...
test('recorded execution should persist the thrown error and preserve rejection when executePlan fails', async () => {
    ...
    // When
    const rejection = await execute({ plan: {}, marathonId: 42 }).catch((caught) => caught);
