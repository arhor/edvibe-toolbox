            ...
            <div class="overlay" data-part="overlay">
                <section class="dialog" data-part="dialog" role="dialog" aria-modal="true" aria-labelledby="history-title">
                    ...
                    <div class="workspace">
                        <aside class="browser-panel">
                            <form class="filters" data-role="filters" @submit=${(event) => {
                                ...
                                <label data-field>
                                    Operation
                                    <select name="operationType" .value=${this.filterOperationType} @change=${(event) => this.setFilter('operationType', event.currentTarget.value)}>
                                ...
                                <label data-field>
                                    Status
                                    <select name="status" .value=${this.filterStatus} @change=${(event) => this.setFilter('status', event.currentTarget.value)}>
                                ...
                                <label data-field>
                                    Marathon
                                    <input name="marathonId" type="search" inputmode="numeric" placeholder="Any marathon" .value=${this.filterMarathonId} @input=${(event) => this.setFilter('marathonId', event.currentTarget.value)}>
                                </label>
                                <div class="date-fields">
                                    <label data-field>
                                        From
                                        <input name="from" type="date" .value=${this.filterFrom} @input=${(event) => this.setFilter('from', event.currentTarget.value)}>
                                    </label>
                                    <label data-field>
                                        To
                                        <input name="to" type="date" .value=${this.filterTo} @input=${(event) => this.setFilter('to', event.currentTarget.value)}>
                                    </label>
                                </div>
                            ...
                            <div class=${stateClass} data-part="empty-state" data-role="state" ?hidden=${!stateVisible}>
                                ${this.listMessage}
                            </div>
                            <div class="record-list" data-role="record-list" ?hidden=${!listVisible}>
                    ...
                    <footer class="dialog-footer">
                        <details class="retention-settings">
                            ...
                            <div class="settings-grid">
                                <label class="checkbox">
                                    <input type="checkbox" name="keepIndefinitely" .checked=${indefinite} @change=${(event) => this.updatePreference('mode', event.currentTarget.checked ? 'indefinite' : 'limits')}>
                                    Keep indefinitely
                                </label>
                                <label data-field>
                                    Newest executions
                                    <input type="number" name="maxCount" min="1" step="1" .value=${String(this.preferences.maxCount)} ?disabled=${indefinite} @input=${(event) => this.updatePreference('maxCount', event.currentTarget.value)}>
                                </label>
                                <label data-field>
                                    Maximum age, days
                                    <input type="number" name="maxAgeDays" min="1" step="1" .value=${String(this.preferences.maxAgeDays)} ?disabled=${indefinite} @input=${(event) => this.updatePreference('maxAgeDays', event.currentTarget.value)}>
                                </label>
                                <label class="checkbox">
                                    <input type="checkbox" name="autoExport" .checked=${this.preferences.autoExport} @change=${(event) => this.updatePreference('autoExport', event.currentTarget.checked)}>
                        ...
                        <p class=${toastClass} data-notice=${this.toastError ? 'danger' : 'success'} data-role="toast" role="status" ?hidden=${!this.toastMessage}>
                            ${this.toastMessage}
                        </p>
                    </footer>
                </section>
