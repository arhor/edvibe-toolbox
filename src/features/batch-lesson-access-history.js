(function initializeBatchLessonAccessHistory(root, factory) {
    if (typeof module === 'object' && module.exports) {
        module.exports = factory(
            ...
            require('./batch-lesson-access-history-model.js'),
            require('./batch-lesson-access-history-record.js'),
            root
        );
    } else {
        root.EdVibeBatchLessonAccessHistory = factory(
            root.EdVibeBatchLessonAccess,
            root.EdVibeBatchLessonAccessHistoryModel,
            root.EdVibeBatchLessonAccessHistoryRecord,
            root
        );
    }
                    ...
                    .then((history) => {
                        ...
                        if (history?.stored) {
                            ...
                            if (history.record?.id) {
                                addHistoryButton(rootObject, dialog, history.record.id, current.stylesheetUrl, openHistory);
                            }
                        } else {
                            appendStatus(dialog, 'Экранный результат сохранён, но записать историю не удалось.', true);
