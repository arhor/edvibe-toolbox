    ...
    function appendPage(items, total, nextItems, nextTotal, label) {
        if (
            ...
            throw createFeatureError(
                'INVALID_RESPONSE',
                `${label} returned invalid pagination data.`
            );
        }
    ...
    function buildUserPlan({ rows }) {
        return (Array.isArray(rows) ? rows : []).map((row) => {
            ...
            const hasCurator = Boolean(
                matched
                && Array.isArray(row.pupil.Moderators)
                && row.pupil.Moderators.length > 0
            );
            return {
                email: row.email,
            ...
            try {
                ...
                if (row.deleteSelected) {
                    if (row.unassign?.status === 'failed') {
                        row.delete = createSkippedOperation(
                            'Skipped because curator removal failed.'
                        );
                    } else {
                        report(row, 'delete');
                        try {
    ...
    function createBatchUserManagementFeature({
        ...
        async function handleCheck(event) {
            ...
            try {
                ...
                currentRows = buildUserPlan({
                    rows: orderResolvedRows(parsed, resolution)
                });
                dialog.showReview({ rows: currentRows });
                log(
                    `Batch user management checked ${currentRows.length} row(s) `
                    + `for MarathonId ${marathonId}.`
                );
            } catch (error) {
